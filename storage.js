...
(function () {
  ...
  var Store = {
    ...
    toRow: function (p) {
      ...
      return {
        numero: p.numero,
        estado: window.Petar.estadoVisible(p),
        fecha: p.identificacion.fecha,
        area: p.identificacion.area,
        subarea: p.identificacion.subarea,
        ubicacion: p.identificacion.ubicacion,
        empresa: p.identificacion.empresa,
        responsable_trabajo: p.identificacion.responsableTrabajo,
        solicitante: p.identificacion.solicitante,
        tipo_trabajo: window.Petar.tipoTrabajoTexto(p),
        equipo: window.Petar.equiposTexto(p),
        elemento: window.Petar.elementoTexto(p),
        descripcion: p.trabajo.descripcion,
        trabajadores: window.Petar.personalTexto(p),
        n_trabajadores: (p.personal.trabajadores || []).length,
        supervisor: p.personal.supervisor,
        vigia: p.personal.vigia.requiere === 'si' ? p.personal.vigia.nombre : 'No requiere',
        vigencia_inicio: v.inicioFecha + ' ' + v.inicioHora,
        vigencia_fin: v.finFecha + ' ' + v.finHora,
        controles_cumplidos: r.si,
        controles_sin_cumplir: r.no,
        criticos_sin_cumplir: r.criticasNo,
        sin_responder: r.sinResponder,
        espacio_confinado: p.especiales.confinado === 'si' ? 'Sí' : 'No',
        recipiente_inflamables: p.especiales.recipienteInflamables === 'si' ? 'Sí' : 'No',
        agentes_exposicion: (p.exposicion.agentes || []).join(' | '),
        observaciones: p.observaciones,
        fotos: (p.fotos || []).length,
        firma_ejecutor: p.autorizacion.ejecutor.nombre,
        firma_autorizante: p.autorizacion.autorizante.nombre,
        suspension_motivo: p.suspension ? p.suspension.motivo : '',
        cierre_fecha: p.cierre.fechaHora,
        cierre_responsable: p.cierre.responsable,
